...
- **`permissions.additionalDirectories`**: the plugin's own install directory (`~/.claude/plugins/cache/mayker/mayker-dev`), so the framework's hook libraries and version file are readable. It is version-independent on purpose — each installed version is a directory inside it — so a `claude plugin update` does not stale the entry. A matching `permissions.deny` rule on `Edit(~/.claude/plugins/cache/**)` keeps it read-only: the cache is read by every session and written by none, and `defaultMode: "acceptEdits"` would otherwise auto-accept an edit there.
