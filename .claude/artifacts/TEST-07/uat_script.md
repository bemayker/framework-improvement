...
| 4.2 | Expand `GET /api/uptime` and read its `200` response | The example body has the keys `uptime_seconds` (a number) and `started_at` (a string of format `date-time`), and the schema link names `UptimeResponse`. The operation takes no parameters | [ ] Pass [ ] Fail |
| 4.3 | Scroll to the `Schemas` section at the bottom of the page and expand `UptimeResponse` | It has exactly two required properties: `uptime_seconds`, of type `number` with minimum `0`, and `started_at`, of type `string` with format `date-time` | [ ] Pass [ ] Fail |
