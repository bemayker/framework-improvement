...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    """Create the notes table and capture the uptime start instant on startup.

    The uptime capture runs before the database branch, so it happens on
    every startup path, including the one where DATABASE_URL is unset.

    Table creation is skipped with a warning when DATABASE_URL is unset: the
    version endpoint needs no database and must still answer, so startup
    never hard-requires one (the notes endpoints then fail loudly per request
    instead).
    """
