<!-- materialized-from: mayker-dev v0.3.58; do not edit, regenerate with /init-project refresh-rules -->
<!--
...
| `reviewer` | Self-review against `review_standards.md`, and a second narrowed pass over what that review could not cover: in `/build-feature` the UAT and documentation artifacts generated after it plus any file the refactor gate added (Phase H), and in `/deliver` 6.5, `/fix` and `/security-fix` the gate's added files alone, dispatched only when it added some (`review_standards.md` Section 6.4) — used by `/build-feature`, `/build-features`, `/fix`, `/security-fix`, `/deliver` | Read-only; cannot edit, run, or commit |
