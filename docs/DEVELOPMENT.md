...
4. **Read the report's closing line.** `/upgrade-project` covers what no re-run heals. `.claude/project_state.json` and these docs are regenerated by a `/sync-project` re-run instead, and an absent `.claude/settings.json` or a missing `.gitignore` entry by an `/init-project` re-run — that `/init-project` re-run only ever **adds what is absent**, so a `permissions` block you already have is never rewritten by it — but a `/sync-project` re-run does reconcile the shipped entries inside that block, adding every grant and denial the installed plugin ships that you are missing and retiring the ones it has withdrawn, while leaving every entry you added yourself untouched; what still reaches you through the ledger is a change to a single-value setting inside `permissions` that you have already set, because your own value is deliberately kept — and the report tells you when they predate the installed plugin. The two commands are complementary; a clean migration list is not the same claim as "fully current".
