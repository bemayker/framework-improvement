...
This framework needs an **issue tracker** MCP connection with the default `Work Item Source: tracker`; with `local` source it needs none. It is defined in `.mcp.json` at the repo root (project scope, committed to git so the whole team shares it).

**The Git provider is not an MCP connection on GitHub, and that is decided by the provider rather than by availability.** On `github` — github.com and GitHub Enterprise alike — every remote git operation goes through the **`gh` CLI**: branches, pushes, PRs, check status, review comments, branch updates, merges and repository creation. So install the GitHub CLI and run `gh auth login` once, or set `GH_TOKEN`; there is no MCP path to fall back to, and a command that needs the remote **stops** with "⛔ **GIT PROVIDER:** GitHub projects require an authenticated `gh` CLI; run `gh auth login` or set `GH_TOKEN`." rather than degrading. Do **not** add a `github` MCP server: nothing reads it.

**On `gitlab` or `bitbucket` the Git provider MCP is the path**, added to `.mcp.json` the same way as the tracker and verified by a functional call. **One consequence is worth knowing before you choose that route:** the framework's branch guard and test gate are `PreToolUse` hooks on `Bash`, so they gate a `git push` and **do not gate a provider MCP's own write tools** — on a GitLab or Bitbucket project a push made through the MCP runs no local suite and is not checked against the branch convention. On GitHub the question does not arise, because every write is a `gh`/`git` Bash call.

For the PR review comments that `/revise-feature` and re-planning read: they come through the same path. Where nothing can reach them, paste the feedback into the task prompt instead.
...
#### Git provider, worked example: GitHub (the `gh` CLI)

There is nothing to add to `.mcp.json`. Install the GitHub CLI and authenticate it once:

```bash
gh auth login          # interactive, once per machine
gh auth status         # what every command checks
```

For an **unattended** surface (Claude Code on the web, a Routine, GitHub Actions, headless `claude -p`, and every `/deliver` run) there is nobody to complete an interactive login, so set **`GH_TOKEN`** in the environment instead. Give the token the scopes the agents need — classic: `repo`, plus `workflow` if features touch `.github/workflows`; fine-grained: Contents, Pull requests and Issues read/write plus Metadata read.

> **The hosted GitHub MCP server is retired here (mayker-dev 0.3.141).** The framework used to add `https://api.githubcopilot.com/mcp/` as a `github` server and route autonomous runs through it exclusively. Two facts ended that: the hosted endpoint serves **github.com only** and returns 404s on every repo call against an Enterprise host while the connection itself looks healthy, which is why the working path had to be probed functionally at all; and a `gh`/`git` write is a `Bash` call, which the framework's branch guard and test gate both gate, while an MCP write is gated only where a hook happens to name the tool. If your `.mcp.json` still declares a `github` server, remove it — `/sync-project` then drops the `mcp__github__*` grant from `.claude/settings.json` on its next run, because that block's MCP entries are derived from `.mcp.json`.

> GitLab and Bitbucket publish their own MCP servers, added the same way as the tracker. If their auth server supports OAuth Dynamic Client Registration, the OAuth flow works; otherwise use the PAT-in-header pattern below.

#### GitHub Enterprise (GHES and ghe.com)

`gh` defaults to github.com, so an Enterprise host needs one setting and one login. **`/init-project` detects a non-github.com GitHub remote and writes `GH_HOST=<your-host>` into the `env` block of `.claude/settings.json`**, so every `gh` call in every session (including cloud ones) targets the right host with no `--hostname` flag. The one manual step:

```bash
gh auth login --hostname <your-host>     # once per machine
```

Unattended, set `GH_HOST` and `GH_TOKEN` in the environment instead.

This is the whole Enterprise story now, and it is the reason the MCP path was retired for GitHub: the hosted MCP endpoint served github.com only and needed a tenant-specific URL or a self-hosted container per Enterprise host, while `gh` needs one environment variable. `project_state.json` → `git_provider.effective_path` still records the resolved path (see below); on a GitHub project it always reads `gh`.
...
OAuth servers (such as the official ClickUp one above) avoid this entirely, since there is no token to store. This pattern applies to any token-authenticated server you add — a GitLab or Bitbucket provider server, or a community tracker server. **The GitHub CLI is not one of them:** `gh` keeps its own credential outside the repo, and `GH_TOKEN` is an environment variable rather than anything `.mcp.json` refers to.
...
- **`permissions.additionalDirectories`**: the plugin's own install directory, named **twice** and deliberately. `~/.claude/plugins/cache/mayker/mayker-dev` covers it on a machine that leaves `CLAUDE_CONFIG_DIR` at its default, and it is version-independent on purpose — each installed version is a directory inside it — so a `claude plugin update` does not stale it. `~/.mayker/mayker-dev` is a symlink the plugin's `plugin-root-link.sh` hook refreshes at every session start, and it is what covers the cache on a machine that sets `CLAUDE_CONFIG_DIR` to anything else, where the first entry names a directory the plugin is not in. **Keep both.** Measured on CLI 2.1.267: an entry spelled `$CLAUDE_CONFIG_DIR/plugins/cache/…` is not expanded and the read stays refused, an entry *is* resolved through a symlink, and a dangling entry is tolerated while the other still binds — so on a non-default config directory the single old entry meant every read of a framework standard, of the version file and of a hook library was silently denied. A `permissions.deny` rule on `Edit(~/.claude/plugins/cache/**)` keeps the default cache read-only under `defaultMode: "acceptEdits"`; there is deliberately no mirrored rule for the second entry, because an additional directory was measured to open a **read** and not an edit (an `Edit` tool call inside a covered cache is refused with no deny rule naming it at all).
- **No bare file-tool grant, and that is what confines a session to this repository.** `permissions.allow` carries no `"Read"`, `"Edit"`, `"Write"`, `"Grep"` or `"Glob"` entry on its own. A permission rule that is a tool name with no parentheses matches *every* use of that tool, so those five — which the block shipped until 0.3.139 — granted the file tools on **any path on disk** while buying nothing inside the repo, where reads need no approval and `acceptEdits` already covers edits. Measured on CLI 2.1.263 with them present: a headless read of a file in `~/Downloads` and of an unrelated sibling repository both succeeded, and a write outside the repo landed on disk. With them gone, all three are refused — in `acceptEdits`, `auto` and `bypassPermissions` alike — while reads, edits and writes inside the repo, reads under `.claude/worktrees/{ID}/` and reads of the plugin cache are all unchanged. **So a session reads this repository and the plugin cache; anything else needs your explicit approval, and an unattended run simply cannot reach it.** Reaching outside deliberately is per-path and never a rule: `--add-dir <path>` at launch, `/add-dir` in a session, or `permissions.additionalDirectories` in your gitignored `.claude/settings.local.json`. **Do not add a bare file-tool grant back as a convenience** — it re-opens the whole filesystem in one line, and a `Read(/some/outside/path/**)` entry is the narrow form if you really need one path.
- **And no bare `"Bash"` grant either, which is the escape the permissions documentation names.** Putting the tool name on its own in `permissions.allow` and denying the few commands you want blocked from a `PreToolUse` hook is a documented pattern, and the framework measured it and declined it. Measured on CLI 2.1.265, 27 rows, one variable: it makes every one of the 45 `Bash(<program>:*)` entries beside it unable to change any verdict; `cd backend && ls`, refused today, becomes allowed, so it reaches outside the working directory the same way the five file-tool grants did; and it does **not** fix the one failure here that is silent — a step-marker append into `.claude/` stays refused under it. `permissions.deny` does keep binding ahead of it (measured in both directions, so `rm -rf` and the force-push flags are unaffected), but `deny` matches command prefixes and the shapes a blanket grant newly permits are exactly the ones no prefix parses. If a program your stack needs is prompting, **declare it in `CLAUDE.md`** and let `/sync-project` derive the `Bash(<program>:*)` grant; that is the supported route and it is auditable. The full reasoning is in `.claude/rules/workflow_triggers.md` Section 5.5.
- **And do not set `permissions.blockReadsOutsideWorkingDirectories`, whatever the settings reference calls it.** It reads like the hardening the first bullet asks for, and it is measured to buy nothing here while breaking the one mechanism that bullet names. Measured on CLI 2.1.263 and re-confirmed on 2.1.266, one variable per row, and **declined**: every outside read it was meant to fence is *already* refused without it, in `acceptEdits`, `auto` and `bypassPermissions` alike, once no bare file-tool grant is present — and it **overrides `permissions.additionalDirectories`**, so a file inside an ordinary additional directory reads fine with the key off and is refused with it on, by the Read tool and by `cat`. Setting it therefore removes the plugin-cache coverage this repo depends on wherever that cache is not inside your active `CLAUDE_CONFIG_DIR`, and the symptom is a framework command that stops rather than one that prompts. It is not a re-trust artifact either: the key-off twin carried the same additional directory under the same already-accepted workspace trust and was allowed. The only difference it makes is attended — an outside read that would prompt you is refused outright instead — which is a confirmation dialog bought by disabling the widening route on every unattended surface. The full reasoning is in `.claude/rules/workflow_triggers.md` Section 5.6.
- **Hooks**: deterministic gates (see [Hooks](#hooks) below).

> **A freshly cloned or newly mounted checkout has NO project permission rules at all, whatever this file ships.** Project `permissions.allow` rules do not apply until Claude Code's workspace trust dialog has been accepted for that folder. Measured: an untrusted clone carrying the full committed block refused even `gh api`, which the block grants outright. So the first interactive session in a new clone must accept the trust prompt, and **every unattended surface that clones fresh** — a scheduled Routine, a GitHub Action, a cloud dispatch, a new worktree mount on a fresh machine — starts with no grants. Trust the folder once, interactively, before pointing an unattended run at it; no framework instruction can substitute for that and none claims to.

> **What none of this reaches: a subprocess that opens files itself.** Read and Edit rules and the working-directory confinement govern the *tools* and Claude Code's recognised read-only shell commands (`cat`, `head`, `tail`, `sed`, …). They do not govern a Python, Node or shell **script** that opens a path of its own — the permission layer sees `python3 script.py`, not the files that script reads. The framework grants interpreters (`python3`, `node`, `bash`) because the lifecycle needs them, so this route is open by construction and **no `.claude/settings.json` entry closes it**. The only mechanism that does is Claude Code's **sandbox** (`sandbox.filesystem.denyRead`, with `allowRead` re-opening the working directory), which applies to Bash subprocesses, is macOS/Linux/WSL2-only, and defaults to read access to the whole machine — turning it on means re-allowing every toolchain that lives under your home directory (`~/.nvm`, `~/.pyenv`, `~/.npm`, `~/.config/gh`, `~/.gitconfig`, `~/.docker`) one at a time, per machine. The framework does not ship it. This paragraph exists so nothing here reads as a claim that the permission layer fences that route: it does not, and the guard for it is the same one as for secrets — keep anything that must not be read out of reach of the machine the session runs on.
...
| --- | --- | --- | --- |
| `CLICKUP_API_KEY` | Auto-transition features to Done on merge | ClickUp → Settings → Apps → API Token | Your Git provider's repository secrets settings |
| `MAYKER_FLIP_TOKEN` | Let the auto-Done job push the work-item file flip to a protected default branch (`local` and `hybrid` source only) | A repository admin's fine-grained PAT with `contents: write` | Your Git provider's repository secrets settings |

> The secret name matches your tracker. ClickUp uses `CLICKUP_API_KEY`; Linear uses `LINEAR_API_KEY`; Jira uses `JIRA_API_TOKEN` **and** `JIRA_EMAIL`. With `Work Item Source: local`, the auto-Done step flips an in-repo file instead of calling a tracker API, so **no tracker secret is required** (the optional Slack secrets below still apply). With `hybrid` the secret **is** required: tracker-resident items are transitioned through the REST API, and without it their authoritative status is only warned about, never flipped. **One more secret applies to `local` and `hybrid` and only when the default branch is protected: `MAYKER_FLIP_TOKEN`, a repository admin's fine-grained PAT with `contents: write`.** The auto-Done job checks out with it so the work-item file flip can push to the protected branch; the built-in `github-actions` bot cannot be given a ruleset bypass (see [Scope of the gates](#hooks)), so this is the only single-secret route, and without it the job warns by name and the flip is rejected.
...
| `.claude/project_state.json` | Pinned tracker identifiers (workspace/project/team) + status mapping + feature registry, each entry recording where its Feature ID came from and where the tracker carries it (`id_source`, `id_carrier`) plus the item's canonical tracker `url` where the tracker's response carried one (tracker source); tracker-less (Git provider + `work_item_source` only) under `local` source. Also records `git_provider.effective_path`: the verified Git-provider working path (`gh` or `mcp`, with host, timestamp, plugin version), so sessions go straight to the path that works. **It is keyed on the provider:** a `github` project always records `gh`, and a record still reading `mcp` there predates mayker-dev 0.3.141 and is re-resolved on the first session after the upgrade — no migration needed, because a plugin version bump already marks every record stale | Generated by `/sync-project`, `effective_path` refreshed by pipeline sessions when stale |
...
> bash ~/.mayker/mayker-dev/hooks/lib/status-mapping-validate.sh .claude/project_state.json
> ```
...
The `mayker-dev` plugin wires eight deterministic hooks in its manifest (`.claude-plugin/plugin.json`). The scripts live in the plugin and the manifest names them with the CLI's own `${CLAUDE_PLUGIN_ROOT}` placeholder, which the CLI expands before it runs a hook; they are **not** files in this repo. A command **you or a session type** reaches the same scripts through `~/.mayker/mayker-dev/`, the version-independent pointer the plugin refreshes at every session start, because the placeholder is refused by the permission layer outside a hook (measured; `rules/workflow_triggers.md` Section 5.1):

- **`branch-guard.sh`** (PreToolUse on `git commit`/`git push`): hard-enforces the branch convention on every commit and push the session makes. It checks the current branch **of the tree that command runs in** — the primary checkout, or the item's worktree when the run uses per-feature worktrees, resolved from a `cd`/`git -C` in the command or the session's directory. **Blocks** on an unrecognized branch (it allows `feature/*`, the standalone `refactor/*` and `test/*` branches that `/refactor` and `/generate-tests` create, the `chore/mayker-*` branches `/upgrade-project`, `/sync-project` and `/deliver` cut for their own generated artifacts, and `main`/`master`/`develop`; **no other `chore/` name is allowed** — the namespace is what makes such a branch traceable to the command that generated it), so cloud surfaces that start on an auto-generated `claude/<slug>` branch cannot commit or push there. The block message is fed back to the agent, which then switches to the feature branch and retries; it names the branch's source in priority order — `feature_map.md`, else the local work item's frontmatter, else `feature/{WORK_ITEM_ID}-{slug}` built from the item's own ID when the repo has neither file — so a repo without a feature map gets a remedy instead of a file to hunt for. **The verdict does not depend on how the command is spelled:** `git commit`, `cd <dir> && git commit` and `git -C <dir> commit` issued from one directory resolve to one tree, including when the session's project directory is itself not a repository (a workspace folder holding several checkouts, a monorepo of separate repos). **Fails open** when it cannot resolve a branch (detached HEAD, no repository at the resolved directory) — but never silently: each of those prints one `[branch-guard] fail-open:` line naming what it could not resolve, because an exit 0 with no output is indistinguishable from a gate that ran and passed. This is what makes the auto-Done pipeline's feature-ID parsing reliable.
- **`test-gate.sh`** (PreToolUse on `git push`): runs the project's test suites before any push, **in the tree the push comes from**. It resolves that tree from the command's own directory (a `cd` or `git -C` in the command, else the session's) — so an autonomous run's `git -C .claude/worktrees/{ID} push` gates the item's worktree, and `Worktrees: per-feature` gates the feature's code rather than whatever the primary checkout happens to hold. A subdirectory of the same checkout is not a different tree: a monorepo push still gates from the repo root. It first honors the explicit **`Test gate command:`** from `CLAUDE.md` → Test Configuration (written by `/sync-project` from the real stack as plain text; the hook strips a bold label, a wrapping code span and a trailing `#` comment before running it, and refuses — blocking the push with the fix — a command whose backticks it cannot resolve, rather than eval'ing something else); only when that is unset/`auto` does it fall back to auto-detection, which scans the repo root **and one level deep** (e.g. `backend/pyproject.toml`, `frontend/package.json`) and runs each suite in its own directory with the right runner (`uv run pytest` when `uv.lock` is present, else `pytest`/`python3 -m pytest`; the package.json `test` script via pnpm/yarn per lockfile, else npm). **Detection is evidence, not a guess:** a Python suite needs a real config file (`pyproject.toml`, `pytest.ini` or `setup.cfg`) at the root just as it does one level deep — a bare `tests/` directory is not evidence of pytest, since it just as easily holds shell scripts or Playwright specs. A repo running pytest with no config file anywhere is therefore detected as nothing, and the gate says so rather than running an empty suite; give it an explicit `Test gate command:`. **Fails closed** (blocks the push) if tests genuinely fail, **and also when tests are detected but no usable runner is on PATH** (with guidance to install the runner or set `Test gate command:`); **fails open** (allows) only when no explicit command is configured and no test setup exists at the root or one level deep (pre-scaffold), when it cannot enter the resolved run directory, or when an auto-detected runner collects no tests (pytest exit 5). That last case is allowed but **never reported as a pass**: it prints a `[test-gate] fail-open:` line naming the tree and the suite's entry in `Suites run:` says `collected no tests`, because a detected suite that collected nothing is as likely to be a mis-detection as an empty one and the two used to be the same green line. A configured `Test gate command:` or `Scoped test command:` is not covered by that: it was written rather than guessed, so its exit code decides the gate as-is and exit 5 **blocks** the push — an empty run of the command your project chose is not a pass, and CI's exit-5 tolerance is a different instrument (it keeps a workflow shared across subprojects from going red, it is not a verdict about your push). **That block says what actually happened**: it names the field, says it `collected no tests`, and names the three ways out (write the tests, fix a filter that currently matches nothing, or point the field at a command that has a suite). It is deliberately not the "tests failed … fix the failing tests" message, which named a cause that did not exist; a real failure still gets that message, unchanged, so the two are distinguishable in the log. **Before any suite runs it exports the run's resolved backing services**: the `- env:` lines of `.claude/artifacts/run/handover/{ID}-run.md` become environment variables for the suites (`{SERVICE}_URL`, `{SERVICE}_PORT`, and `DATABASE_URL`/`TEST_DATABASE_URL` for a relational engine), so the host-assigned port a build provisioned on is visible to the gate and a build whose every tier passed inside the dispatches is not blocked at the push by a suite falling back to `localhost:5432`. Variable names are logged, never values. **Only the current run's lines are exported**: that record is keyed by work item and grows across runs, so the hook derives the current run as the last `run=` value in the file and skips every line carrying another id or none, counting them on stderr — a previous run's service is a container that was torn down. A record with no `run=` key anywhere predates that convention: nothing in it can be attributed, so nothing is exported and the hook says so. **And which record is chosen comes from the branch, not from modification time:** it is `{ID}-run.md` for the item the `feature/{ID}-{slug}` branch names, so another item's record is never this item's environment — several items share one Docker host by design. **A push that names no item is not part of any lifecycle run** — a maintenance push on `main`, a revert, a `refactor/*`, `test/*` or `chore/mayker-*` branch — and it exports nothing at all, running against this project's declared environment or its defaults; the gate says so on the `[test-gate] env:` channel, as a decision rather than a fail-open. **Every remaining service is then connection-tested before it is exported**, one TCP connect bounded to a second: the record is written when a service comes up and is never rewritten, so it cannot know the run's Summary tore the container down afterwards. A closed port is skipped and named on stderr with its service, endpoint, run and `teardown=` value, and never exported; a line the gate cannot probe at all is exported with that stated, because "cannot check" is not "not there". **If a suite then fails after this gate exported something, the block names it** — the variables, the record, the run and the endpoint — so an environment failure is not diagnosed as a test failure. The failure sentence itself is unchanged. No record at all is the normal case for a project with no backing services and changes nothing; a record line it cannot use prints a `[test-gate] fail-open:` line naming the cause and the suites run anyway. Every run names the suites it executed, and every fail-open prints a `[test-gate] fail-open:` line naming the tree it searched, so a passing gate is auditable against a gate that ran nothing — and "no tests here" cannot hide the case where it looked in the wrong tree. **A push whose diff is non-code runs no test command at all.** Before it reads the run record and before it resolves any command, the gate classifies the paths this push would add to the remote: under `docs/`, under `.claude/`, under `.github/`, or ending in `.md` is non-code, and **everything else is code — including `.github/workflows/pr-tests.yml`, the one carve-out**, which is the only workflow whose edit the tiers it defines actually exercise. All non-code means the gate exits 0 having run nothing, printing `[test-gate] skip:` with the reason and the paths, so a documentation-only plan push no longer runs your whole suite for a `.md` file. It is the same list `pr-tests.yml`'s change gate applies in CI, deliberately, so the two cannot disagree about one diff. **The list is shared; the diff range is not:** this gate reads `@{u}..HEAD`, because it makes a claim about the push it is about to allow, while CI reads the PR's three-dot diff, because a required check makes a claim about the head. **One code path is enough to run the normal gate**, and the skip errs that way everywhere: the diff range is `@{u}..HEAD`, or the default branch when the branch has no upstream yet — and **the remote that names it is resolved rather than assumed** (`hooks/lib/remote-resolve.sh`), so the candidates are `{remote}/HEAD`, `{remote}/main`, `{remote}/master` and then the `origin/*` spellings. Until 0.3.180 all three were `origin` literals, so on a fork, a mirror or any clone whose remote is named otherwise the skip never fired at all and your whole suite ran on every documentation-only first push. A range that cannot be resolved, or resolves to no changed path, runs the suite instead of skipping, and the reason names every candidate tried. A hook never fetches. The skip is reported as a **decision** and never through the fail-open channel: "the gate chose to run nothing" and "the gate could not run anything" are different facts and must not read alike. **And every push's verdict is written where it can be read after the push, since 0.3.120.** Everything above is stderr, and a `PreToolUse` hook's stderr on exit 0 is not shown to the session, so on the success path the gate's verdict scrolled past and was gone. The gate now appends one line to the same run record it reads, as a third kind beside `- tests:` and `- env:`: `- gate: run={id} phase=push head={sha} verdict={skipped|scoped|full|incomplete|none} duration={n}s suites="..." reason={token}`. It is a separate kind rather than another `- tests:` line on purpose — those are the reviewer's observed test tiers, and a hook-written verdict in that set would read as a tier it never ran. The line is written only where a lifecycle run owns the push (the branch names a work item, that item's record exists, it carries a `run=`, and that run is still being appended to), so a maintenance push records nothing; **the fourth condition is there because the run id is derived** — it is the record's last `run=` value, since a hook is not in a session and cannot be handed one, which is right for a build's own push and wrong for a push made by a command that owns no line in the record at all, so a re-plan of an already-built item used to append its verdict under the finished build's id and inflate that run's row count with a push it never made; past the bound the verdict goes to stderr alone and names the run the gate declined to attribute the push to; a blocked push records nothing either, because its block message already reaches the agent. Every success path also prints one `[test-gate] verdict:` line naming the verdict, the gate's own duration and whether it was recorded. Read the lines back with `~/.mayker/mayker-dev/hooks/lib/gate-verdicts.sh`, which prints one row per push and one `[gate-verdicts]` summary line last. **And since 0.3.135 every lifecycle push checks that its own verdict is there.** `head=` is the `HEAD` of the tree the gate judged, which is what makes a line addressable to one **push** rather than to a run: a run pushes more than once (the build push plus one per CI fix), so counting its push-phase lines answers nothing about the last push. Each lifecycle command chains `~/.mayker/mayker-dev/hooks/lib/gate-assert.sh <record> <run id>` onto its push command and prints the one line it returns on its `Gate verdict:` summary field: `test-gate: verdict recorded for this push (head 1a2b3c4: full)`, or `test-gate: no verdict recorded for this push (head 1a2b3c4)`. **The second line does not mean the tests failed — it means this push was not gated at all**, so no local suite verified it and CI is its only authority; read it exactly as you would a `[test-gate] fail-open:` line, never as a green gate. **Since 0.3.170 it also answers the case where `HEAD` moved between the gate and the check** (MDF-209): a `PreToolUse` hook fires once, before the whole Bash call, so a push command that chains a commit ahead of itself is gated at the parent of the commit it publishes, and matching on the post-commit head alone reported a verdict missing that was in the record. The helper now also matches a gate line of this run whose head is an ancestor of the pushed head **and which this very push published** — established against the upstream's own reflog, so an earlier push's line can never stand in — and says so in its line: `test-gate: verdict recorded for this push (gated head 1a2b3c4: full; pushed head 9f8e7d6 -- HEAD moved after the gate ran ...)`. Where it cannot establish that, it claims nothing and reports the push as unverdicted. **A fourth line, `test-gate: cannot assert this push's verdict: ...`, is not the third one**: it says this check could not be made (an unusable tree, argument or record) rather than that this push was not gated, and only `no verdict recorded for this push` counts a push as unverdicted. The helper reports and never gates: it exits 0 on every path, including a usage error, because it runs after the push in the same command and its exit status is what the session reads as the push's own. It exists because a push whose gate never fired left a record byte-identical to a push it passed, and that happened for real: at 0.3.132 both push hooks were inert on the framework's own multi-line push commands, two pushes recorded no verdict, and nothing said so. **And `full` is a claim the run has to earn** (MDF-217): a suite that ran but collected no tests verified nothing, so a push where any selected suite came back empty records `verdict=incomplete reason=collected-no-tests` carrying `verified=N hollow=N`, never `full`. The push is still allowed, because that fail-open is MDF-074's and is unchanged; what stops is the record claiming a full suite ran, since `gate-assert.sh` prints that claim onto the `Gate verdict:` line of every report and PR body, where it is read as evidence.
...
> **Scope of the gates.** `branch-guard` and `test-gate` are `PreToolUse` hooks on **`Bash`**, so every push the framework makes is gated: shell `git` in assisted mode, and `git -C .claude/worktrees/{ID} push` in an autonomous run — since mayker-dev 0.3.141 those are one path rather than two, because remote git on GitHub goes through `gh`/`git` and no longer through an MCP. A push made through some other API path would not pass through them. **On a GitLab or Bitbucket project that is a real gap and it is stated rather than hidden:** the provider MCP's own write tools are not matched by either hook, so a push made through them runs no local suite and is not checked against the branch convention. Treat these hooks as a strong in-session guard, and keep the CI checks (PR tests, auto-Done branch match) as the backstop.
>
...
>
> **So `/sync-project` recommends one, and it never applies one without your explicit approval.** Its Section P reads your default branch's protection on every run, in both project modes, and where the branch is unprotected it proposes a ruleset: require a pull request (the only way onto the branch — a direct push is rejected), leave approving reviews optional (0 required — anyone can review, no review blocks a merge), dismiss stale approvals on new commits, require the `pr-tests` status check (plus the Aikido check when Security Scanning is on — the same required-status-check setting the [security scanning guide](https://github.com/bemayker/mayker-dev-plugin/blob/main/docs/security-scanning.md) describes, carried by one ruleset rather than two unrelated rules), block force-push and deletion, and restrict bypass to repository admins **and to no app at all**. That clause named the `github-actions` bot until mayker-dev 0.3.149 and no longer can: **GitHub does not offer the built-in Actions identity as a ruleset bypass actor** (measured 2026-09-03, re-checked against GitHub's own changelog and community discussion #13836 on 2026-09-04; `/sync-project` Section P step 4 carries the sources). Under Work Item Source `local` or `hybrid` the auto-Done pipeline pushes the work-item file flip straight to the default branch, so on a protected branch that push needs a credential instead: the repository secret **`MAYKER_FLIP_TOKEN`**, a repository admin's fine-grained PAT with `contents: write`, which the workflow checks out with and which bypasses through the admin role the ruleset already carries. Without it merged items silently stay in their pre-merge status (see [Notes & edge cases → When auto-Done fails after a merge](#when-auto-done-fails-after-a-merge)), and `/sync-project` Section P reports the gap on every run under those two sources. It requires `pr-tests` only once that check has actually **reported** on a real pull request in your repository — a requirement pointed at a check that never reports blocks every merge forever — so a brand-new repo gets the rest of the ruleset now and the check requirement on the next `/sync-project`. **The review count stays at 0 for a reason, and raising it is a real trade in autonomous mode:** requiring an approving review makes an unattended `/deliver` merge impossible by design, since the run cannot approve its own PR and will leave every item green and unmerged. That is presented as a choice rather than decided for you — require a review and merge by hand, or leave reviews optional and keep `/deliver` autonomous.
...
| `/waves` | haiku | Renders the grouped wave view; reads two files and writes nothing |
...
The per-step **observed** model in `.claude/artifacts/{ID}/stats_summary.md` is the only place that answers "which model ran": a step marker's `model` field is the framework's declaration of intent, and the collector checks it against what ran INSIDE that step's window rather than believing it — the dispatch's own transcript where the window brackets one, the session's turns where it brackets none, and nothing where it brackets a dispatch the collection could not read, which is named as unchecked instead of flagged (plugin 0.3.169, MDF-201). Before that it was always compared against the dispatching session, so the marker the rule mandates on a bracketed dispatch flagged every time and a dispatch that silently ran a tier down was averaged away. **Since plugin 0.3.116 a marker declares one only for a step that brackets a subagent dispatch**, where the value is that agent's route-independent `agents/*.md` pin. A step the dispatching session ran itself carries no `model` at all: a command pin reaches one route only, no session can read the model it is itself running on, and writing the pin there produced a `model!=marker` flag on every unpinned route — which is a marker-writer defect, not an expected flag, and suppressing a working detector by explaining it away is the direction that is never taken (MDF-135).
...
4. Analyzes dependencies (direct `depends_on` per feature) and proposes a `wave` for every unwaved row, which you approve or amend
...
9. Commits whatever it wrote to a `chore/mayker-sync-{version}-{date}` branch and opens one pull request whose body is this run's own report (Section G)

Running it twice on an unchanged repo is a clean no-op, and it only ever rewrites content it discovered — never a section you filled in by hand. A no-op run therefore cuts no branch and opens no pull request, and its report says so on the `Maintenance PR:` line.

> **Its output arrives as a pull request, and that is a change from how this command used to behave.** Before mayker-dev 0.3.143 both maintenance commands left every generated file as an uncommitted change on whatever branch you happened to be on — in practice the default branch, which is exactly the direct-push route the ruleset in [Scope of the gates](#hooks) exists to close. They now commit only the files the run itself wrote (never `git add -A`, so your own uncommitted edits are never swept into a framework pull request) onto a `chore/mayker-*` branch that `branch-guard.sh` allows by name. **No other `chore/` branch is allowed**, so this is not a general-purpose escape hatch: a hand-made `chore/bump-postgres-18` is still refused, and your own maintenance work still goes on a `feature/` branch. Nothing here merges anything — the pull request is yours to review.
...
When re-planning (dispatched again for a feature with an existing plan): reads PR review comments through the Git provider working path and revises the plan to address the feedback.
...
1. Reads PR review comments through the Git provider working path
...
- `/waves [wave N]`: prints the grouped wave view of `feature_map.md` — one block per wave with each item's dependencies, flags and live status, `Unwaved` last. Read-only: it renders and writes nothing, stores no status anywhere, and never edits a cell.
...
1. Verifies the autonomy gate and the Git provider working path (hard-required, no degrade: an authenticated `gh` on a GitHub project, the provider MCP on GitLab or Bitbucket)
...
5. Per item: plan (planner) → self-approve → build + tests (builder, own worktree) → self-review (reviewer) → refactor gate → re-check the files that gate added, if it added a source or test file (reviewer) → commit + push from the worktree + draft PR through the working path → poll CI → fix failures → convert the PR to ready for review and set In Review once green → address review comments → merge on the framework's own verdict → verify Done → unblock dependents
...
- **Headless `claude -p`**: orchestrate dispatch from your own infrastructure. **On an unattended surface, name the command in its namespaced form, `/mayker-dev:<command>`:** the bare `/<command>` spelling resolves only in an interactive session, and `claude -p` answers an unknown command by printing `Unknown command` and exiting **0**, so an unattended caller must check the run's own recorded product, the artifact or the report header its instruction names, and never the exit code alone.
...
| Remote git | The Git provider working path: `gh` on GitHub, the provider MCP on GitLab/Bitbucket | The same path, and no degrade: the run stops at setup if it cannot be verified |
| Quality gates | test gate, branch guard, self-review, refactor gate | identical, plus the same gates enforced on MCP pushes |

**One-time setup (the only manual step).** Fill in `CLAUDE.md` (tech stack, MCP configuration, test configuration) and the Architecture Notes, set `Autonomy: autonomous` in the `## Autonomy` section, make the Git provider path work headlessly (`GH_TOKEN` for a GitHub project — an unattended run cannot complete an interactive `gh auth login`; the provider MCP on GitLab or Bitbucket) and connect the tracker MCP for `tracker`/`hybrid` source, commit, push. `/init-project` and `/sync-project` are optional: a first `/deliver` performs both halves non-interactively and records its choices in `DECISIONS.md`.
...
**Repositories.** Work lands on `feature/{ID}-{slug}` branches of this repo by default. An item that is a genuinely separately-deployable service gets its own repository under the configured organization (created with `gh repo create` and initialized through a clone, recorded in `project_state.json` → `repositories`); ambiguous cases stay branches here.
...
5. Review and merge the `chore/mayker-sync-*` pull request `/sync-project` opened, then **commit and push whatever it did not carry** — `/init-project`'s own output (`.gitignore`, `.claude/rules/`, and `CLAUDE.md` if nothing rewrote it) is not part of that pull request, because Section G commits only the files that run itself wrote (cloud sessions clone all of it)
...
- **Wave:** Which front a feature belongs to — authored by you, proposed by `/sync-project`, and an ordering key only

Readiness is computed from the graph (an item is ready when all its `depends_on` are Done, `.claude/rules/work_items.md` Section 7); the `wave` column groups the rows for a human and never gates one. Agents read this file to check dependencies. They then verify actual status via MCP.

**`wave` is where you record sequencing intent, and it is the only thing in this file you write for yourself.** The graph says what blocks what; it cannot say "these six are this fortnight's front, and item Z is in it". Put a positive integer in the `wave` cell and the batch commands and `/deliver` take lower waves first: `/build-features wave 2` selects that front, and `/deliver` dispatches the lowest wave first **among items that are already ready**. It is an ordering key and never a gate — a ready wave-3 item is still dispatched when nothing in wave 1 is ready, and a genuine "not before that" belongs in `depends_on`. One rule is enforced: every dependency of a waved row must itself be waved and strictly smaller, so you can pull an item forward only as far as the graph allows; the validator names the edge if you cannot. An empty cell means unwaved and is perfectly valid — a map with no waves at all passes. **No status is ever stored here**: run `/waves` for the grouped view with live status, as often as you like. Local work items (`docs/issues/{ID}.md`) carry `wave: N` in their frontmatter instead.

**`test_checkpoint` is the one authored column that is not about dependencies.** The pre-push gate runs a *scoped* suite on a feature branch and CI runs everything against one PR at a time, so nothing ever runs the whole suite against a locally integrated `main`. Put a `✅` on the row whose merge closes a group of work and the full suite (`CLAUDE.md` → `Full suite command:`, falling back to `Test gate command:`) runs once at that point: reported and non-blocking in `/build-feature` and `/build-features`, blocking in `/deliver`, which admits no newly unblocked item after a red one. **Any number of rows may carry it**, unlike `scaffold`, and `/sync-project` proposes candidates (sinks, and items that unblock three or more others) for you to accept or amend. A candidate you decline is written back as `➖`, which runs nothing and is simply the decision recorded: it stops the same proposal coming back on every re-run, and **clearing the cell is how you ask to be proposed a value again**. Leave every cell empty and nothing runs — a project with no meaningful boundary is a legitimate answer.
...
- A first `changes` job works out whether **this PR** contains code
...
> **Documentation-only and configuration-only PRs run no tests, and no path filter is involved.** The `changes` job diffs the PR against its merge base (`base...head`, three-dot, on every event type) and skips every tier when nothing outside `docs/`, `.claude/`, `.github/` and `*.md` changed — which is what stops a plan PR, or one carrying only run statistics or a workflow tweak, from running the whole matrix. `.github/workflows/pr-tests.yml` is the one carve-out and counts as code, because it is the only workflow whose edit the tiers it defines actually exercise. **It asks about the PR rather than about the push, and that is the whole of its correctness:** the workflow cancels a superseded run, a cancelled run leaves no conclusion behind, and a required check is read per head commit — so a per-push gate would let a docs-only push landing seconds after a code push report green with every tier skipped over code nothing had tested. A `paths-ignore:` filter is still not how any of this is done: it would leave the required check pending, which blocks the merge. Anything the gate cannot resolve (a missing SHA, a git failure) runs every tier.
...
5. Review and merge the `chore/mayker-sync-*` pull request `/sync-project` opened, then commit and push whatever it did not carry: Section G commits only the files that run itself wrote, so `/init-project`'s own output is still yours to commit.
...
2. Run `/upgrade-project --dry-run` locally, read the report, then run `/upgrade-project`. It re-materializes the seven always-on rules into `.claude/rules/` (each stamped with the generating plugin version), then applies the plugin's migration ledger: one **targeted edit** per outstanding change, so your customized CI workflows and your own `CLAUDE.md` content survive. Anything it will not do automatically is reported as `MANUAL` with the exact text and the command to inspect, and it never prompts, so it is also safe in a Routine or a GitHub Action. **Its writes land there too, and that is a separate guarantee from never prompting:** re-materializing the rules writes into `.claude/`, where Claude Code auto-declines a tool-call write on an unattended session (Routine, GitHub Action, headless `claude -p`) — above `.claude/settings.json`, above `--allowedTools`, above any hook, so no permission setting turns it on. The plugin therefore performs those writes from inside its own script, whose writes are not adjudicated, and a **stale** repo upgrades on an unattended surface rather than stopping. It still stops rather than reporting an upgrade it did not perform, and the cause left is a folder whose workspace trust dialog has never been accepted: that withholds every project permission rule, and the fix is to open the folder interactively once.
3. **Review the pull request it opened.** Since mayker-dev 0.3.143 `/upgrade-project` does not leave its output as an uncommitted diff: it commits exactly the files it wrote to a `chore/mayker-upgrade-{from}-to-{to}` branch and opens one pull request whose body is that run's own migration report. Read it, merge it, and cloud sessions pick up the new standards from the default branch. A run that changed nothing cuts no branch and says so; on a repo with no remote or no verified Git-provider path the branch and commit are still made locally and the report names the one step that finishes it. `--dry-run` never branches and never commits, and names the branch it would have cut.
4. **Read the report's closing line.** `/upgrade-project` covers what no re-run heals. `.claude/project_state.json` and these docs are regenerated by a `/sync-project` re-run instead, an absent `.claude/settings.json` by an `/init-project` re-run, and a missing `.gitignore` entry by a `/sync-project` re-run (its Section 5.2, since 0.3.171, reconciles the framework's ignore entries on every run and leaves every line you own byte-for-byte) — that `/init-project` re-run only ever **adds what is absent**, so a `permissions` block you already have is never rewritten by it — but a `/sync-project` re-run does reconcile the shipped entries inside that block, adding every grant and denial the installed plugin ships that you are missing and retiring the ones it has withdrawn, while leaving every entry you added yourself untouched; what still reaches you through the ledger is a change to a single-value setting inside `permissions` that you have already set, because your own value is deliberately kept — and the report tells you when they predate the installed plugin. The two commands are complementary; a clean migration list is not the same claim as "fully current".
   - **The boundary is per artifact, not per file, and two files sit on both sides of it.** A full re-run also re-derives `CLAUDE.md` → Test Configuration → **Backing Services** in full, fills a **`Test gate command:`** that is missing, a placeholder, or `auto`, and since mayker-dev 0.3.179 rewrites `.claude/feature_map.md`'s **`## Schema`** block from the plugin's own template (`/sync-project` Section 5.3, every run, both modes — your rows and every other line of the map are left byte-for-byte). Those three blocks therefore never get a migration entry, so a clean migration list says nothing about them — which is exactly the case where a `/upgrade-project` report alone would leave you believing your `CLAUDE.md` is current. Every *other* field in `CLAUDE.md`, and every other part of the map's structure, does reach you through the ledger.
...
- `autonomous`: `/deliver` drives the whole backlog per `.claude/rules/autonomy.md`; see [Autonomous mode](#autonomous-mode-deliver). Combines freely with either Project Mode and any Work Item Source (a headlessly usable Git provider path becomes hard-required).
...
**A command stops with "GitHub projects require an authenticated `gh` CLI".** `gh` is the whole Git provider path on GitHub (mayker-dev 0.3.141), so this is a setup failure with no fallback. Run `gh auth login` — or `gh auth login --hostname <your-host>` on Enterprise — and check `gh auth status`. On an unattended surface set `GH_TOKEN` (and `GH_HOST` on Enterprise) in the environment instead: nobody there can finish an interactive login. If you previously configured a `github` MCP server in `.mcp.json`, remove it; nothing reads it, and `/sync-project` then drops the now-unused `mcp__github__*` grant from `.claude/settings.json`.
...
**A pipeline command stops with "Run /sync-project first".** `.claude/project_state.json` is missing or was never committed. Run `/sync-project` locally (`/init-project` first if the repo has no `CLAUDE.md`), then commit and push the generated `.claude/` files: cloud sessions clone the repo and need them present. (`/diagnose`, `/fix`, `/watch-pr`, `/security-scan`, `/security-fix` and `/waves` are the commands that do not require `project_state.json`.)
...
The auto-Done pipeline runs in CI, not in Claude Code, so it uses the tracker REST API (not MCP). It matches the `feature/{FEATURE_ID}-{slug}` branch name. The `/plan-feature` and `/build-feature` skills set this branch name, and the `branch-guard.sh` hook hard-enforces it in-session by blocking commits/pushes on an unrecognized branch (it allows `feature/*` plus the `refactor/*` and `test/*` prefixes the standalone commands use, the maintenance commands' `chore/mayker-*` branches, and the base branches), even when the dispatch surface starts the session on an auto-generated `claude/<slug>` branch (Claude Code on the web and Routines). If you intentionally dispatch with a different prefix, adjust `branch-guard.sh` and align the branch regex in the auto-Done pipeline accordingly.
...
4. **Local work-item source:** the equivalent failure is the file-flip commit not landing, most often because the default branch is protected and the token the job pushed with is not allowed to. The job logs `::warning::MAYKER_FLIP_TOKEN is not set` when that is the cause. Add a repository admin's fine-grained PAT with `contents: write` as the `MAYKER_FLIP_TOKEN` secret (GitLab/Bitbucket: grant the CI user push access to the protected branch), then re-run the job; or set `status: done` in `docs/issues/{ID}.md` by hand and commit it with `[skip ci]`. **Exempting the `github-actions` bot in the ruleset is not an option** — GitHub does not offer the built-in Actions identity as a bypass actor. A custom GitHub App installed on the repository and listed in `bypass_actors` is the other route that works, and the framework does not ship it: an App has no static token, so it is an app id, a private key and a token-minting step per run rather than one secret.
...
Use the `shared_risk_notes` column in `feature_map.md` to spot the file overlaps most likely to conflict: review those features' PRs together and merge the lower-risk one first. If two independent features touch the same files heavily, sequence them (merge one, rebase the other) rather than building both blindly in parallel. In autonomous mode the `/deliver` scheduler enforces exactly this: flagged pairs are serialized, and a stale or conflicting PR branch is updated through the Git provider working path (`gh api …/pulls/{n}/update-branch`, or merged locally in the item's worktree) before the merge verdict.
