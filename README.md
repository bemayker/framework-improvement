...
1. From the repository root, start all services: `docker compose up --build`. No `.env` file is needed: every value `docker-compose.yml` reads has a working default for the Compose network. The `--build` matters after a feature adds a backend dependency (the backend image installs them at build time), so reusing a stale image fails at import; plain `docker compose up` is fine once the image is current.
...
`.env.example` is the template for running a service **directly on the host**, where `DATABASE_URL` must point at `localhost`. Compose also reads a root `.env` for variable interpolation, so if you keep one, set `DATABASE_URL=postgresql://tasknotes:tasknotes@db:5432/tasknotes` (the `db` service name) before `docker compose up`, or the backend container resolves `localhost` to itself and cannot reach the database.
...
├── .env.example                    # Env var template (DB connection + frontend API base URL); for host-run services, not needed by Compose
