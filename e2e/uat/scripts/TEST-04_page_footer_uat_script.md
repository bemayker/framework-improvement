...
- Have the version the backend reports to hand: run `curl -s http://localhost:8010/api/version` and read its `version` field — the footer's expected version comes from that response, not from this document and not from `frontend/package.json`. At the time of writing it is `0.1.0`; a later version bump changes the expected text without changing this script.
...
| 4 | Compare the version number in the footer to the `version` field returned by `curl -s http://localhost:8010/api/version` | The two values match exactly | [ ] Pass [ ] Fail |
